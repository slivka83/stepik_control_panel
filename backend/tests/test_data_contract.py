...
SNAPSHOT_TOP_KEYS = {"summary", "months", "courses", "recent_payments", "promos", "community"}

SUMMARY_KEYS = {
    "total_turnover", "total_income", "total_refunds", "total_payments",
    "total_refunds_count", "net_income", "current_month_turnover",
    "current_month_income", "current_month_payments",
}

COURSE_KEYS = {"course_id", "title", "price", "turnover", "income", "refunds", "payments"}

MONTH_KEYS = {"month", "year", "month_num", "turnover", "income", "refunds",
              "payments_count", "refunds_count"}

COMMUNITY_KEYS = {
    "average_rating", "total_reviews", "total_comments", "comments_monthly",
    "total_solutions", "solutions_monthly", "per_course",
}
...
COURSES_API_FIELDS = {
    "id", "stepik_course_id", "title", "status", "price", "income",
    "published_at", "enrollment_count", "submissions_total",
    "submissions_correct", "comments_count", "reviews_count", "average_rating",
}
...
async def _seed_full_pipeline(session):
    ...
    for cid, title, price in [(101, "Python 101", 2990), (102, "JS Basics", 1990)]:
        ...
        """), {
            "cid": cid, "title": title, "bpa": "2026-01-15T00:00:00Z",
            "raw": json.dumps({
                "id": cid, "course_id": cid, "title": title, "price": price,
                "is_public": True, "became_published_at": "2026-01-15T00:00:00Z",
            }),
        })
    ...
    await session.execute(text("""
        ...
        VALUES (1001, 101, 85, :lv, :dj, '{}'),
               (1002, 101, 60, :lv2, :dj2, '{}')
    """), {
        "lv": (now - timedelta(days=2)).isoformat(), "dj": (now - timedelta(days=30)).isoformat(),
        "lv2": (now - timedelta(days=10)).isoformat(), "dj2": (now - timedelta(days=40)).isoformat(),
    })
    await session.execute(text("""
    ...
    """), {
        ...
        "j1": json.dumps({"year": now.year, "month": now.month, "total_turnover": 10000,
                          "total_user_income": 8000, "total_refunds": 200,
                          "count_payments": 10, "count_refunds": 1}),
        "y2": prev_year, "m2": prev_month,
        "j2": json.dumps({"year": prev_year, "month": prev_month, "total_turnover": 5000,
                          "total_user_income": 4000, "total_refunds": 100,
                          "count_payments": 5, "count_refunds": 0}),
    })
    ...
    """), {
        ...
        "j1": json.dumps({"course": 101, "amount": 1000, "payment_amount": 1200,
                          "status": "completed", "time": (now - timedelta(days=3)).isoformat(),
                          "buyer": 1001, "promo_code": None, "currency_code": "RUB"}),
        "j2": json.dumps({"course": 101, "amount": -200, "payment_amount": 1200,
                          "status": "refunded", "time": (now - timedelta(days=4)).isoformat(),
                          "buyer": 1002, "promo_code": "DISCOUNT10", "currency_code": "RUB"}),
        "j3": json.dumps({"course": 102, "amount": 1500, "payment_amount": 1500,
                          "status": "completed", "time": (now - timedelta(days=5)).isoformat(),
                          "buyer": 1003, "promo_code": None, "currency_code": "RUB"}),
    })
    ...
    """), {
        ...
        "j2": json.dumps({"user": 2, "target": 500, "time": (now - timedelta(days=7)).isoformat(), "thread": "solutions"}),
    })
    await session.commit()

    from app.services.transform import (
        transform_courses, transform_enrollments, transform_submissions,
        transform_financials, transform_community,
    )
...
async def test_snapshot_schema_is_complete(db_session):
    ...
    assert SNAPSHOT_TOP_KEYS <= set(data), f"missing top-level keys: {SNAPSHOT_TOP_KEYS - set(data)}"
    assert SUMMARY_KEYS <= set(data["summary"]), f"missing summary keys: {SUMMARY_KEYS - set(data['summary'])}"
    assert COMMUNITY_KEYS <= set(data["community"]), f"missing community keys: {COMMUNITY_KEYS - set(data['community'])}"

    assert len(data["courses"]) == 2
    for c in data["courses"]:
        assert COURSE_KEYS <= set(c), f"missing course keys: {COURSE_KEYS - set(c)}"
    for m in data["months"]:
        assert MONTH_KEYS <= set(m), f"missing month keys: {MONTH_KEYS - set(m)}"
    for cid_str, pc in data["community"]["per_course"].items():
        assert PER_COURSE_KEYS <= set(pc), f"per_course[{cid_str}] missing keys: {PER_COURSE_KEYS - set(pc)}"
...
async def test_every_course_has_price_and_per_course_entry(db_session):
    ...
    for cid, course in by_id.items():
        ...
        assert PER_COURSE_KEYS <= set(pc), f"course {cid} per_course incomplete: {pc}"
...
async def test_rerun_pipeline_keeps_full_schema(db_session):
    ...
    from app.services.transform import transform_financials, transform_community
    await transform_financials(db_session)
    await transform_community(db_session)
    await db_session.commit()

    data = await _get_snapshot(db_session)
    assert SNAPSHOT_TOP_KEYS <= set(data)
    assert SUMMARY_KEYS <= set(data["summary"])
    assert COMMUNITY_KEYS <= set(data["community"])
    for c in data["courses"]:
        assert COURSE_KEYS <= set(c)
...
async def test_courses_api_returns_every_frontend_field(db_session):
    ...
    try:
        ...
        for c in courses:
            assert COURSES_API_FIELDS <= set(c), f"missing API fields: {COURSES_API_FIELDS - set(c)}"
