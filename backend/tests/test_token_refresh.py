...
class TestTokenRefresh:
    ...
    async def test_refreshes_expiring_token(self, db_session):
        ...
        expiring = (datetime.now(timezone.utc) + timedelta(minutes=5)).replace(tzinfo=None)
        user = User(
            id=uuid.uuid4(), stepik_id=456,
