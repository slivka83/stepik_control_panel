...
export default function Financials() {
  ...
  const paginatedPayments = (recent_payments || []).slice((page - 1) * rowsPerPage, page * rowsPerPage);
              ...
              <tbody>
                {paginatedCourses.map((c) => (
                  <tr key={`course-${c.course_id}`} className="border-b border-gray-800">
                    <td className="py-2 text-white truncate" title={c.title}>
                      {c.title}
                    </td>
              ...
              <tbody>
                {paginatedPayments.map((p) => (
                  <tr key={`payment-${p.id}`} className="border-b border-gray-800">
                    ...
                    <td className="py-2 text-white truncate pr-4" title={p.course}>
                      {p.course}
                    </td>
                    <td className="py-2 text-right text-gray-300 truncate pr-4" title={p.student || ''}>
                      {p.student || '—'}
                    </td>
                    <td className="py-2 text-right font-mono text-white truncate pr-4">
                      {formatCurrency(p.payment_amount)}
                    </td>
                    <td
                      className={`py-2 text-right font-mono truncate pr-4 ${p.status === 'refunded' ? 'text-crimson-alert line-through' : 'text-neon-green'}`}
                    >
