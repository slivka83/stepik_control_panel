...
describe('Students', () => {
  ...
  it('does not render list header and total counter', () => {
    render(
      <TestRouter
        syncValue={makeSyncValue({ active: 1, passive: 0, fading: 0, sleeping: 0 }, { students: [], total: 7677 })}
      >
        <Students />
      </TestRouter>,
    );
  ...
  it('renders aggregated student rows', () => {
    ...
    render(
      <TestRouter syncValue={makeSyncValue({ active: 1, passive: 1, fading: 0, sleeping: 0 }, studentsData)}>
        <Students />
      </TestRouter>,
    );
  ...
  it('links student name to profile', () => {
    const studentsData = {
      students: [makeStudent({ student_id: 777, profile_url: 'https://stepik.org/users/777', name: 'Мария Смирнова' })],
      total: 1,
    };
    render(
      <TestRouter syncValue={makeSyncValue({ active: 1, passive: 0, fading: 0, sleeping: 0 }, studentsData)}>
        <Students />
      </TestRouter>,
    );
